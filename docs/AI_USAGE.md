...
- **「AIがMCPで書き、人間がVS Codeで仕上げる」運用の場合** → `excel_update_module_code`の確定書き込み直後に`excel_export_module`でそのモジュールをエクスポートフォルダへ書き出す。これを怠ると、人間が**古いファイル**を編集し、後の手動Importで**AIの修正が黙って巻き戻る**（この運用で最も危険な取りこぼし）。エクスポートは`.Export()`によるExcelの現在状態の書き出しなので、Attribute行・`.frm`/`.frx`ペアとも手動エクスポートと同品質。コード本文は応答に含まれない（ローカルファイルにredactionを通さないための意図的な設計）。逆方向（編集済みファイル→Excel）は、拡張機能側の保存時プロンプト（設定`excelVbaSync.importPromptOnSave`）が人間に提案する。**人間とAIが同じモジュールを同時に触った場合**：前回のエクスポート以降にファイルが変更されていると`ERR_EXPORTED_FILE_MODIFIED`で上書きを拒否する（＝人間の未インポート編集を検知）。エージェントはユーザーに確認し、先にインポートするか、破棄してよいと確認できたときだけ`force:true`で再呼び出しする（その場合も上書き前バックアップは取られる）。判定はエクスポート時に記録するハッシュ（サイドカー）との照合で、手動の「Export All Modules」はサイドカーを更新しないため、手動エクスポート直後のMCPエクスポートは誤検知することがある（余計に確認する方向の誤りなので安全側）
