...
`dryRun`レスポンスに含まれる。`newCode`内のPublic相当（Private以外）のSub/Function/Propertyの名前が、プロジェクト内の他モジュールに既に存在していないかを照合する（種別は区別しない：他モジュールのFunctionと同名のSubを書いても検出される）。対象モジュール自身との重複は対象外。実際に「Functionをクラスモジュールへ移そうとしたが元の標準モジュールに同名Functionが残ってしまった」という実害を受けて追加した。書き込みはブロックしない（advisory）。既存モジュール上書き・新規作成のどちらのモードでも動作するが、判定のためにプロジェクト全体を追加で読み込む分、`dryRun`呼び出しごとにCOM往復が1回増える。
