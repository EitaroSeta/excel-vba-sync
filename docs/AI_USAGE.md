...
3. AIクライアントを再起動すると、`ping` / `excel_list_modules` / `excel_get_module_code` / `excel_list_macros` / `excel_run_macro` / `vba_search_code` / `excel_update_module_code` / `excel_read_range`の8ツールが使えるようになる
...
**MCPサーバー内部での多重起動対策（v0.0.52で対応済み）**
「モジュール一覧・マクロ一覧・セル値を一度に教えて」のような複数部分からなる依頼で、クライアント側が複数のMCPツールを**同時（並列）に呼び出す**と、以前は各呼び出しが独立にExcelの起動状態を確認するため、お互いの起動処理が終わる前に「Excelが起動していない」と判定してしまい、**1回の依頼でExcelプロセスが複数（例: 3つ）立ってしまう**という不具合がありました。現在はサーバー内部でExcel/COMに触れる処理を1つずつ順番に実行するようキュー化しており、複数ツールを同時に呼び出しても新規Excel起動は最初の1回だけになります（実機で3並列呼び出し→Excelプロセス1つのみを確認済み）。
...
上記の通り`excel_run_macro`は「例外を投げずに完走した」ことしか保証しません。`excel_read_range`はマクロ実行後に**実際のセル値を読み取って結果を自己検証する**ために使えます（例：セル操作マクロを書く→実行する→対象範囲を読んで期待通りの値になっているか確認する→違えばコードを直して再実行、というループ）。

- シート名（`sheet`）とセル範囲（`range`、例: `"A1:C10"`）を指定して値を取得する読み取り専用ツール
- `Range.Value2`を使用（`.Value`と違い日付・通貨型への暗黙変換が起きない）
- 行×列の2次元配列で返る（単一セルでも`[[値]]`という1x1の配列になる）
- **VBAプロジェクトオブジェクトモデルへのアクセス（Trust Center設定）は不要**：VBAコードのツール群と異なり、通常のExcelオブジェクトモデル（Range/Worksheet）しか使わないため
- 列全体・行全体（`"A:A"`等）のような広すぎる範囲は遅くなりうるので、`"A1:C10"`のように範囲を絞ることを推奨
- 書き込み側（セルへの値の書き込み）は現時点では未実装（読み取り専用）
